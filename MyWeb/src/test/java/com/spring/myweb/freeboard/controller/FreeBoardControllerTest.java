...
public class FreeBoardControllerTest {
	...
	// 구현 할 수 있는 가상의 구조를 만들어 줍니다.
	private MockMvc mockMvc;
	
	@BeforeEach //테스트 시작 시 다른 메서드 실행 전 항상 먼저 구동되는 기능들을 선언
	public void setup() {
		// 가상 구조를 세팅
	...
	@Test
	@DisplayName("3번글 상세 보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 "
			+ "model에 담아서 jsp로 이동시킬 것이다.")
	
	// /freeboard/freeDetail -> get
	void testContent() throws Exception {
		ModelAndView mv = mockMvc.perform(
				MockMvcRequestBuilders
				.get("/freeboard/content")
				.param("bno", "3"))
...
//		String viewPage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/delete")
//				.param("bno", "3"))
//				.andReturn().getModelAndView().getViewName();
