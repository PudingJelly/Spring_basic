		...
		<div class="row">
			<div class="col-lg-6 col-md-9 col-sm-12 join-form">
				...
				<form action="">
					...
					<div class="form-group">
						<!--기본 폼그룹을 가져온다-->
						<label for="password">비밀번호</label> <input type="password" class="form-control" id="userPw"
							placeholder="비밀번호 (영 대/소문자, 숫자 조합 8~16자 이상)"> <span id="msgPw"></span>
						<!--자바스크립트에서 추가-->
					</div>
					<div class="form-group">
...
<script>
		...
		*/
		...
		fetch('${pageContext.request.contextPath}/user/idCheck', reqObj)
			...
			.then(data => { // 텍스트만 뺀 Promise 객체로부터 data 전달받음.
				if (data === 'ok') {
					// 더이상 아이디를 작성 할 수 없도록 막아주겠다.
					document.getElementById('userId').setAttribute('readonly', true);
					// 더이상 버튼을 누를 수 없도록 버튼 비활성화.
					document.getElementById('idCheckBtn').setAttribute('disable', true);
					// 메세지 남기기
	...
	document.querySelector('.mail-check-input').onblur = function (e) {
		...
		const $resultMsg = document.getElementById('mail-check-warn'); //span 태그
		...
		if (inputCode === code) {
			...
			// 초기값을 사용자가 선택한 값으로 무조건 설정하는 방법(select에서 readonly 처럼 사용)
			// 항상 2개 같이 쓰세요
			...
			$resultMsg.textContent = '인증번호를 다시 확인 해주세요.';
			$resultMsg.style.color = 'red';
			e.target.focus(); // 다시 입력할 수 있도록 포커싱 주기.
		}
	} // 인증번호 검증 끝

	// 카카오 주소 api 사용해 보기 (script src 추가 필수!)
