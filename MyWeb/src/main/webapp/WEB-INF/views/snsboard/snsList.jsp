...
<body>
	...
	<script>
		...
		function regist() {
			...
			FormData 객체를 활용합니다.
			무조건 이 방식이 옳은 것은 아닙니다. FomrData는 비동기 방식이 꼭 필요한 경우에만 사용합니다.
			대부분의 경우에는 form 태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용됩니다.
			*/
			const formData = new FormData();
			const $data = document.getElementById('file'); // 이미지 첨부한 input 태그
			...
			// FileList는 유사 배열이기 때문에 인덱스를 이용하여 여러 파일 중 하나를 지목할 수 있습니다.
			// 현재 우리는 multiple이 없기 때문에 [0]이라고 작성하면 파일 정보를 얻을 수 있습니다.
				...
				.then(list => {
					...
					for (vo of list) {
						...
						<div class="link-inner">
							...
							<a id="comment" data-bno="` + vo.bno + `"><i class="glyphicon glyphicon-comment"></i>댓글달기</a>
		...
		document.getElementById('contentDiv').addEventListener('click', e => {
			...
			// 값을 제 위치에 배치하시고 모달을 열어 주세요.(부트스트랩 모달이기 때문에 jQuery로 열기)
		...
		// 서버쪽에서 권한을 확인 해 주세요. (작성자와 로그인 중인 사용자의 id를 비교해서 일치 하는지의 여부)
