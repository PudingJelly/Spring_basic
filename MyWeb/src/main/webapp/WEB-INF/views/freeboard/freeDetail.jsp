    ...
    window.onload = function () {
        ...
        document.getElementById('replyRegist').onclick = () => {
            ...
            fetch('${pageContext.request.contextPath}/reply/regist', reqObj)
                ...
                .then(data => {
                    ...
                    document.getElementById('reply').value = ''; // 텍스트 에어리어 비워두기
                    document.getElementById('replyId').value = '';
                    document.getElementById('replyPw').value = '';
                    // 등록 완료 후 댓글 목록 함수 호출하여 비동기식으로 목록 표현.
                    getList(1, true); // 페이지 번호와 리셋의 여부 전달
                });
        } // 댓글 등록 이벤트 끝.
        ...
        function getList(pageNum, reset) {
            ...
            fetch('${pageContext.request.contextPath}/reply/getList/' + bno + '/' + pageNum)
                ...
                .then(data => {
                    ...
                    // if(replyList.length < 0) return; 또는 
                    ...
                    if(reset) {
                        ...
                        page = 1;
                        if(replyList.length <= 0) return; //이렇게 작성
                    }
        ...
        */ 
        ...
        function parseTime(regDateTime) {
            ...
            const date = new Date();
            console.log(date);
            const gap = date.getTime() - regTime.getTime(); // 밀리초
