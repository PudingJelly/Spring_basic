...
public class SnsBoardController {
	...
	*/
	...
	public ResponseEntity<byte[]> getFile(@PathVariable String fileLoca, @PathVariable String fileName) {
		...
		try {
			// probeContentType: 매개값으로 전달 받은 파일의 타입이 무엇인지를 문자열로 반환.
			// 사용자에게 보여주고자 하는 데이터가 어던 파일인지에 다라 응답 상태 코드를 다르게 리턴하고
	...
	public String delete(@PathVariable int bno, HttpSession session) {
		...
		// 글이 삭제 되었다면 더이상 이미지도 존재할 필요가 없으므로
		// 이미지도 함께 지목해서 삭제.
	...
	public ResponseEntity<byte[]> download(@PathVariable String fileLoca, 
		...
		try {
			...
			e.printStackTrace();
			result = new ResponseEntity<>(HttpStatus.INSUFFICIENT_STORAGE);
		}
		return result;
	}
