...
public class kakaoService {
	...
	public String getAccessToken(HttpSession session, String code, String state) {
		...
		/* CallBack으로 전달 받은 세션 검증용 난수값과 세션에 저장되어 있는 값이 일치하는지 확인 */
		...
		if(sessionValue.equals(state)) { // 로그인 할 때 만든 state와 성공 후 응답된 state가 일치한다면?
			...
			// 카카오 서버쪽에서 설정 해 달라고 한 content-type 설정하기.
	...
	public KakaoUserVO getUserProfile(String accessToken) {
		...
		// 요청 보내기
		RestTemplate template = new RestTemplate();
		ResponseEntity<KakaoUserVO> responseEntity 
				= template.exchange(
				requestUri,
				HttpMethod.GET,
		...
		connection.setRequestProperty("Autorization", "Bearer " + accessToken);
